...
plots <- lapply(data_vec_cleaned, function(d) {
  ...
  plot <-d|>
    ...
    scale_x_continuous(d$Estimates, limits = c(min(d$Estimates), max(d$Estimates)+20))+
    stat_summary(fun = mean,geom = "text", aes(label=round(..y..,2)), size=3, vjust = -0.5, col="white")+
